...
int Main(int argc, char **argv) {
    ...
    // Pre-emptively block the following signals so that we can spawn one thread
    // that cleanly handles all of them:
    // - SIGINT, SIGTERM, SIGHUP: log the interrupt,, Shutdown the threadpool, and exit immediately
    // - SIGUSR1: Sent by main() at the end of normal execution - just shutdown the thread pool and let main() do the rest

    sigset_t blockedSignalSet;
    sigemptyset(&blockedSignalSet);
    for (auto s: {SIGINT, SIGTERM, SIGUSR1, SIGHUP}) {
        sigaddset(&blockedSignalSet, s);
    }
    int rc = pthread_sigmask(SIG_BLOCK, &blockedSignalSet, nullptr);
    if (rc != 0) {
        ERR("Failed to establish signal handlers: (" << ") " << strerror(rc));
        std::exit(rc);
    }
    ...
    if (P4API::ClientSpec.mapping.empty()) {
        ERR("Received a client spec with no mappings. Did you use the correct corresponding P4PORT for the " +
            P4API::ClientSpec.client + " client spec?");
        return 1;
    }

    PRINT("Updated client workspace view " << P4API::ClientSpec.client << " with " << P4API::ClientSpec.mapping.size()
                                           << " mappings");
    ...
    if (!p4.IsDepotPathUnderClientSpec(depotPath)) {
        ERR("The depot path specified is not under the " << P4API::ClientSpec.client
                                                         << " client spec. Consider changing the client spec so that it does. Exiting.");
        return 1;
    }
    ...
    if (git.IsHEADExists()) {
        if (!git.IsRepositoryClonedFrom(depotPath)) {
            ERR("Git repository at " << srcPath << " was not initially cloned with depotPath = " << depotPath
                                     << ". Exiting.");
            return 1;
        }
    ...
    SUCCESS("Found " << changes.size() << " uncloned CLs starting from CL " << changes.front().number << " to CL "
                     << changes.back().number);
    ...
    auto signal_handler_thread = std::thread([&blockedSignalSet, &pool]() {
        int sig = 0;
        int rc = sigwait(&blockedSignalSet, &sig);
        if (rc != 0) {
            ERR("(signal handler) failed to sigwait , ERR (" << errno << "): " << strerror(errno));
            std::exit(rc);
        };

        if (sig == SIGUSR1) {
            // gracefully shutdown - no need to log
            return;
        }

        if (!sigismember(&blockedSignalSet, sig)) {
            ERR("(signal handler) received unexpected signal \"" << strsignal(sig)
                                                                 << "\" this probably indicates a logic error.")
        }

        ERR("(signal handler) Received signal (" << sig << ") \"" << strsignal(sig)
    ...
    for (size_t currentCL = 0; currentCL < startupDownloadsCount; currentCL++) {
        ...
        pool.AddJob([&downloaded, &cl, &branchSet, printBatch](P4API *p4) { cl.PrepareDownload(p4, branchSet); });
    }
    for (size_t currentCL = 0; currentCL < startupDownloadsCount; currentCL++) {
        ChangeList &cl = changes.at(currentCL);
    ...
    SUCCESS("Queued first " << startupDownloadsCount << " CLs up until CL "
                            << changes.at(startupDownloadsCount - 1).number << " for downloading");
    ...
    for (size_t i = 0; i < changes.size(); i++) {
        ...
        SUCCESS(
                "CL " << cl.number << " with "
                      ...
                      << ((commitTimer.GetTimeS() / 60.0f) / (float) (i + 1)) * (changes.size() - i - 1)
                      << " mins left.");
    ...
    SUCCESS("Completed conversion of " << changes.size() << " CLs in " << programTimer.GetTimeS() / 60.0f
                                       << " minutes, taking " << commitTimer.GetTimeS() / 60.0f << " to commit CLs");

    pool.ShutDown();

    // Send a signal to the signal-handling thread to tell it to stop, and then wait for it to finish.
